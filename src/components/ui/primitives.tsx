  ...
  return (
    <div
      className={cn(
        ...
        'text-[#DEA93E] pl-3 border-l-2 border-[#DEA93E] mb-4',
        className
      )}
    >
      {children}
 ...
 *   <span className="text-xl font-display text-[#004D40]">{value}</span>
 * </div>
 */
export function GoldStatItem({
  ...
  return (
    <div className={cn('flex flex-col', className)}>
      <span className="text-[9px] font-black uppercase text-[#004D40]/30 dark:text-white/30 tracking-[0.15em] mb-0.5">
        {label}
      </span>
      <span className="text-xl font-display text-[#004D40] dark:text-white/90">
        {value}
      </span>
    </div>
  );
    ...
    <div className={cn('flex gap-8 items-center', className)}>
      {stats.map((s, i) => (
        <React.Fragment key={s.label}>
          {i > 0 && (
            <div className="w-px h-6 bg-[#004D40]/5 dark:bg-white/10 shrink-0" />
          )}
...
export function EmptyState({
  ...
  return (
    <div className={cn('flex flex-col items-center justify-center py-16 gap-4 text-center', className)}>
      {icon && (
        <div className="h-16 w-16 rounded-2xl bg-[#004D40]/5 dark:bg-white/5 flex items-center justify-center text-[#DEA93E]">
          {icon}
        </div>
      )}
      <div>
        <p className="font-black text-[#004D40] dark:text-white/90 text-base">{title}</p>
        {description && (
          <p className="text-[11px] text-[#004D40]/40 dark:text-white/25 mt-1 font-medium">{description}</p>
        )}
      </div>
    </div>
  );
...
export function IslamicDivider({ className }: { className?: string }) {
  return (
    <div className={cn('flex items-center gap-4 py-4', className)}>
      <div className="h-px flex-1 bg-gradient-to-r from-transparent to-[#004D40]/10 dark:to-white/10" />
      <div className="flex gap-1.5 items-center opacity-40">
        <div className="h-1 w-1 rounded-full bg-[#DEA93E]" />
        <div className="h-2 w-2 rotate-45 border border-[#DEA93E] bg-white dark:bg-black/20 shadow-[0_0_8px_rgba(222,169,62,0.3)]" />
        <div className="h-1 w-1 rounded-full bg-[#DEA93E]" />
      </div>
      <div className="h-px flex-1 bg-gradient-to-l from-transparent to-[#004D40]/10 dark:to-white/10" />
    </div>
  );
}
...
export function UpwardShootingStars() {
  ...
  return (
    <div ref={containerRef} className="absolute inset-0 pointer-events-none overflow-hidden rounded-[32px]">
      <div className="absolute inset-0 bg-[#DEA93E]/[0.02]" />
      <div className="luxury-star-sparkles opacity-50" />

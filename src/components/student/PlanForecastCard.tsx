            ...
            <div className="glass-card-inner !p-6 sm:!p-8">
                ...
                <div className="flex items-start justify-between relative z-10 mb-6">
                    <div className="space-y-1">
                        <h3 className="text-xl font-black tracking-tight text-[#004D40]">{tGlobal('Hifdh Prognose')}</h3>
                        <p className="text-[10px] uppercase font-bold tracking-[0.2em] text-[#DEA93E]">{tGlobal('Tidslinje for fuldførelse')}</p>
                    </div>
                    <div className={cn(
                        "p-3 rounded-2xl shadow-sm",
                        isAhead ? "bg-emerald-500 text-white" : 
                        isOnTrack ? "bg-[#DEA93E] text-white" : 
                ...
                {/* Main Graph */}
                <div className="relative h-44 w-full mb-8 z-10 group bg-[#004D40]/[0.02] rounded-3xl border border-[#004D40]/5 p-4">
                    <svg viewBox="0 0 100 100" className="w-full h-full overflow-visible" preserveAspectRatio="none">
                        {/* Grid lines */}
                        <line x1="0" y1="0" x2="100" y2="0" stroke="currentColor" strokeWidth="0.5" className="text-[#004D40]/5" />
                        <line x1="0" y1="50" x2="100" y2="50" stroke="currentColor" strokeWidth="0.5" className="text-[#004D40]/5" />
                        <line x1="0" y1="100" x2="100" y2="100" stroke="currentColor" strokeWidth="1" className="text-[#004D40]/10" />
                                ...
                                {(() => {
                                    ...
                                    return (
                                        <circle 
                                            ...
                                            cy={lastPoint.y} 
                                            r="2.5" 
                                            className="fill-[#DEA93E] animate-pulse"
                                        />
                                    );
                                })()}
                            </>
                    ...
                    {/* Labels for graph */}
                    <div className="absolute bottom-1 left-2 text-[8px] font-black uppercase text-[#004D40]/20 tracking-tighter">{tGlobal('Start')}</div>
                    <div className="absolute top-1 right-2 text-[8px] font-black uppercase text-[#DEA93E]/40 tracking-tighter">{tGlobal('Mål')} (604p)</div>
                ...
                <div className="space-y-6 relative z-10">
                    <div className="grid grid-cols-2 gap-8 px-2">
                        <div>
                            <p className="text-[10px] font-black uppercase text-[#004D40]/30 tracking-widest mb-1.5">{tGlobal('Forventet færdig')}</p>
                            <p className={cn(
                        ...
                        <div className="text-right">
                            <p className="text-[10px] font-black uppercase text-[#004D40]/30 tracking-widest mb-1.5">{tGlobal('Planlagt mål')}</p>
                            <p className="text-lg font-display text-[#DEA93E]">
                    ...
                    <div className="space-y-3">
                         <div className="flex items-center gap-2 text-[10px] font-black uppercase tracking-widest text-[#004D40]/40 mb-1">
                            <Info className="h-3.5 w-3.5" />
                            {tGlobal('Prognosen er baseret på din gennemsnitlige pace de sidste 4 uger.')}
                        </div>
                        <div className="h-3 w-full bg-[#004D40]/5 rounded-full overflow-hidden p-[2px] border border-white/20">
                            <motion.div 
                                initial={{ width: 0 }}
                                animate={{ width: `${(forecast.completedPages / TOTAL_PAGES) * 100}%` }}
                                transition={{ duration: 1.5, ease: "circOut" }}
                                className="h-full bg-gradient-to-r from-[#DEA93E] to-[#CC9933] rounded-full shadow-md" 
                            />
                        </div>
                        <div className="flex justify-between items-center text-[9px] font-black uppercase tracking-widest text-[#004D40]/30">
                            <span>{forecast.completedPages} {tGlobal('sider fuldført')}</span>
                            <span>{604 - forecast.completedPages} {tGlobal('sider tilbage')}</span>
                        </div>
                    </div>

                    <div className="grid grid-cols-2 gap-3 pt-6 border-t border-[#004D40]/5">
                        <div className="flex items-center gap-3 p-3 rounded-2xl bg-[#004D40]/[0.02] border border-[#004D40]/5">
                            <div className="p-2 bg-white/60 rounded-xl shadow-sm">
                                <CalendarDays className="h-4 w-4 text-[#004D40]/60" />
                            </div>
                            <div>
                                <p className="text-[9px] font-black text-[#004D40]/30 uppercase tracking-widest">{tGlobal('Tempo')}</p>
                                <p className="text-xs font-black text-[#004D40]">{(forecast.actualPace4Weeks * 7).toFixed(1)} {tGlobal('sider/uge')}</p>
                            </div>
                        </div>
                        <div className="flex items-center gap-3 p-3 rounded-2xl bg-[#004D40]/[0.02] border border-[#004D40]/5">
                            <div className="p-2 bg-white/60 rounded-xl shadow-sm">
                                <AlertCircle className="h-4 w-4 text-[#004D40]/60" />
                            </div>
                            <div>
                                <p className="text-[9px] font-black text-[#004D40]/30 uppercase tracking-widest">{tGlobal('Resterende')}</p>
                                <p className="text-xs font-black text-[#004D40]">{forecast.daysRemaining} {tGlobal('Dage')}</p>
                            </div>
                        </div>
                    </div>
                </div>

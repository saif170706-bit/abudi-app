...
const translations: Record<string, Record<Language, string>> = {
  ...
  newSurvey: { da: 'Ny Undersøgelse', en: 'New Survey', ar: 'استبيان جديد' , so: "Sahan Cusub"},
  ...
  responsesCount: { da: '{count} besvarelser', en: '{count} responses', ar: '{count} ردود' },
};
...
export default function AdminPosts() {
  ...
  const resultsRef = useMemoFirebase(() => {
    if (!overviewItem || !firestore) return null;
    const pathMap: any = { event: 'events', survey: 'surveys' };
    const col = pathMap[overviewItem.type];
    const subCol = overviewItem.type === 'event' ? 'registrations' : 'responses';
    return col ? collection(firestore, col, overviewItem.id, subCol) : null;
  }, [overviewItem, firestore]);

  const { data: resultsData, isLoading: isResultsLoading } = useCollection<any>(resultsRef);

  const surveyResults = useMemo(() => {
    if (!overviewItem || overviewItem.type !== 'survey' || !resultsData) return null;

    return overviewItem.questions.map((q: any) => {
      const allAnswers = resultsData
      ...
      if (q.type === 'scale') {
        ...
        aggregation = {
          average: allAnswers.length > 0 ? sum / allAnswers.length : 0,
          distribution: counts,
        ...
        aggregation = { counts, total: allAnswers.length };
      }

      return { ...q, answers: allAnswers, aggregation };
    });
  }, [overviewItem, resultsData]);

  const eventResults = useMemo(() => {
    if (!overviewItem || overviewItem.type !== 'event' || !resultsData) return { appUsers: [], externalUsers: [] };

    const filtered = resultsData.filter(r => {
      const term = regSearchTerm.toLowerCase().trim();
      if (!term) return true;
    ...
    return {
      appUsers: filtered.filter(r => !!r.userId),
      externalUsers: filtered.filter(r => !r.userId),
    };
  }, [overviewItem, resultsData, regSearchTerm]);
  ...
  const handleViewOverview = (item: any) => {
    setTimeout(() => {
      triggerHaptic();
      setRegSearchTerm('');
  ...
  const buildTargetData = () => {
    let finalGender = targetGender;
    if (targetAudience === 'man') finalGender = 'man';
    else if (targetAudience === 'woman') finalGender = 'woman';
    else if (targetAudience === 'specific') finalGender = 'all';
  ...
  const notifyTarget = async (type: any, title: string) => {
    try {
  ...
  const handleCreateOrUpdateAnnounce = async () => {
    ...
    try {
      ...
      if (editingItem) {
        ...
        await notifyTarget('announcement', annTitle);
  ...
  const handleCreateOrUpdateEvent = async () => {
    ...
    try {
      ...
      if (editingItem) {
        ...
        await notifyTarget('event', evtTitle);
  ...
  const handleCreateOrUpdateSurvey = async () => {
    ...
    try {
      ...
      if (editingItem) {
        ...
        await notifyTarget('survey', surveyTitle);
  ...
  const handleCreateOrUpdateMeeting = async () => {
    ...
    try {
      ...
      if (editingItem) {
        ...
        await notifyTarget('livestream', mtTitle);
  ...
  const renderRegistrationCard = (res: any) => (
    <Card key={res.id} className="rounded-2xl border border-border bg-card p-4 flex items-center justify-between shadow-sm">
      <div className="flex items-center gap-3 min-w-0">
        <Avatar className="h-10 w-10 border border-border shadow-sm">
          <AvatarImage src={res.photoURL || undefined} />
          <AvatarFallback>{getInitials(res.userName)}</AvatarFallback>
        </Avatar>
        <div className="min-w-0">
          <p className="text-[15px] font-bold text-foreground truncate">{res.userName}</p>
          <p className="text-xs text-muted-foreground font-medium truncate">{res.userEmail}</p>
        </div>
      </div>
      ...
      <Button
        variant="ghost"
        size="sm"
        onClick={() => setSelectedResultForAnswers(res)}
        className="h-9 px-3 rounded-xl bg-muted hover:bg-black/[0.06] font-bold text-xs"
      >
        {t('viewAnswers')}
      </Button>
    </Card>
  ...
  return (
    <div className="min-h-screen bg-background px-4 pt-10 pb-32 sm:px-6">
      <div className="max-w-xl mx-auto">
        ...
        <div className="grid grid-cols-4 gap-2 mb-10">
          ...
          <button onClick={() => { triggerHaptic(); resetForms(); setIsMeetingOpen(true); }} className="h-24 flex flex-col items-center justify-center gap-2 rounded-[24px] bg-card border border-border shadow-sm text-foreground hover:bg-muted transition-all active:scale-[0.98]">
            ...
            <span className="font-bold text-[9px] uppercase tracking-wider">{t('newMeeting')}</span>
          </button>
        </div>
          ...
          {isFeedLoading ? (
            ...
            feedItems.map(item => (
              <div key={item.id} className="relative group">
                {item.type === 'event' ? (
                  <AdminEventCard event={item as any} onEdit={handleOpenEdit} onViewOverview={handleViewOverview} />
                      ...
                      <div className="flex justify-between items-start gap-4">
                        ...
                        <DropdownMenu>
                          ...
                          <DropdownMenuContent align="end" className="rounded-2xl border-border shadow-xl">
                            {(item.type === 'survey' || item.type === 'event') && (
                              <DropdownMenuItem onSelect={() => handleViewOverview(item)} className="rounded-xl h-11 font-bold gap-3">
                                <BarChart2Icon className="h-4 w-4" /> {item.type === 'survey' ? 'Resultater' : 'Oversigt'}
                              </DropdownMenuItem>
                            )}
      ...
      {/* OVERVIEW SHEET (For Registrations/Survey Results) */}
      <FullscreenSheet
        open={isOverviewOpen}
        onOpenChange={setIsOverviewOpen}
        title={overviewItem?.title || t('eventOverview')}
        rightSlot={<button onClick={() => setIsOverviewOpen(false)} className="p-2 hover:bg-black/5 rounded-full transition-colors"><X className="h-6 w-6 opacity-40" /></button>}
      >
        <div className="p-6 space-y-8 pb-32 mx-auto max-w-2xl">
          {/* Header Summary */}
          <div className="flex flex-col items-center text-center gap-2 mb-2">
            <div className="grid h-16 w-16 place-items-center rounded-2xl bg-blue-50 text-blue-600 shadow-sm border border-blue-100/50">
              {overviewItem?.type === 'survey' ? <BarChart2 className="h-8 w-8" /> : <UsersIcon className="h-8 w-8" />}
            </div>

            <h2 className="text-2xl font-extrabold text-foreground">
              {overviewItem?.type === 'event'
                ? `${(eventResults.appUsers.length + eventResults.externalUsers.length) || 0} / ${overviewItem?.capacity || '∞'}`
                : t('responsesCount', { count: resultsData?.length || 0 })
              }
            </h2>

            <p className="text-sm font-bold text-muted-foreground uppercase tracking-widest">
              {overviewItem?.type === 'event' ? t('totalParticipants') : t('totalResponses')}
            </p>
          </div>
          ...
          {isResultsLoading ? (
            ...
            <>
              {/* SURVEY RESULTS */}
              {overviewItem?.type === 'survey' && surveyResults && (
                <div className="space-y-10">
                  {surveyResults.map((q: any) => (
                    <div key={q.id} className="space-y-4">
                      <div className="px-1 border-l-4 border-primary pl-4">
                        ...
                        <Badge variant="secondary" className="mt-2 bg-muted text-muted-foreground border-none font-bold text-[10px] uppercase">
                          {q.type === 'scale' ? t('scaleField') : q.type === 'text' ? t('textField') : t('choiceField')}
                        </Badge>
                      </div>

                      {q.type === 'scale' && q.aggregation && (
                        <Card className="p-6 rounded-[32px] border-border bg-card shadow-sm">
                          <div className="flex items-center justify-between mb-6">
                            <span className="text-sm font-bold text-muted-foreground uppercase tracking-wider">{t('averageScore')}</span>
                            <span className={cn("text-3xl font-extrabold", getScoreColor(q.aggregation.average))}>
                              {q.aggregation.average.toFixed(1)}
                            </span>
                          </div>

                          <div className="space-y-3">
                            {[5, 4, 3, 2, 1].map(val => {
                              const count = q.aggregation.distribution[val - 1];
                              const percent = q.aggregation.total > 0 ? (count / q.aggregation.total) * 100 : 0;
                              return (
                                <div key={val} className="flex items-center gap-3">
                                  <span className="w-3 text-xs font-bold text-muted-foreground">{val}</span>
                                  <div className="flex-1 h-2 bg-muted rounded-full overflow-hidden">
                                    <div className={cn("h-full transition-all duration-1000", getBarColor(val))} style={{ width: `${percent}%` }} />
                                  </div>
                      ...
                      {q.type === 'text' && (
                        <div className="grid gap-3">
                          {q.answers.length > 0 ? q.answers.map((ans: string, i: number) => (
                            <div key={i} className="p-5 rounded-[24px] bg-card border border-border shadow-sm text-sm leading-relaxed italic">
                              "{ans}"
                            </div>
                          )) : (
                            <p className="text-center py-4 text-xs text-muted-foreground">{t('noResponses')}</p>
                          )}
                        </div>
                      )}

                      {(q.type === 'radio' || q.type === 'checkbox') && q.aggregation && (
                        <Card className="p-6 rounded-[32px] border-border bg-card shadow-sm space-y-4">
                          {Object.entries(q.aggregation.counts).map(([opt, count]: [string, any]) => {
                            const percent = q.aggregation.total > 0 ? (count / q.aggregation.total) * 100 : 0;
                            return (
                              <div key={opt} className="space-y-1.5">
                                ...
                                <div className="h-2 bg-muted rounded-full overflow-hidden">
                                  <div className="h-full bg-blue-500/60" style={{ width: `${percent}%` }} />
                                </div>
                              </div>
                            );
                          })}
              ...
              {overviewItem?.type === 'event' && (
                <div className="space-y-8">
                  ...
                  {eventResults.appUsers.length === 0 && eventResults.externalUsers.length === 0 ? (
                    <div className="text-center py-12 bg-card rounded-3xl border border-border shadow-sm">
                      <p className="text-sm text-muted-foreground">{t('noRegistrations')}</p>
                    </div>
                  ) : (
                    <div className="space-y-10">
                      {/* App Users Group */}
                      {(overviewItem.allowExternalRegistrations || eventResults.appUsers.length > 0) && (
                        <div className="space-y-4">
                          <h3 className="flex items-center gap-2 px-1 text-[13px] font-bold uppercase tracking-widest text-muted-foreground">
                            <UserIcon className="h-4 w-4" />
                            App Brugere
                          </h3>
                          {eventResults.appUsers.length > 0 ? (
                            <div className="space-y-3">
                              {eventResults.appUsers.map(renderRegistrationCard)}
                            </div>
                          ) : (
                            <div className="p-6 text-center border-2 border-dashed border-border rounded-[24px]">
                                <p className="text-xs text-muted-foreground font-medium">Ingen tilmeldte app-brugere</p>
                            </div>
                          )}
                        </div>
                      )}
                      ...
                      {(overviewItem.allowExternalRegistrations || eventResults.externalUsers.length > 0) && (
                        <div className="space-y-4">
                          <h3 className="flex items-center gap-2 px-1 text-[13px] font-bold uppercase tracking-widest text-muted-foreground">
                            <LinkIcon className="h-4 w-4" />
                            Eksterne Tilmeldinger
                          </h3>
                          {eventResults.externalUsers.length > 0 ? (
                            <div className="space-y-3">
                              {eventResults.externalUsers.map(renderRegistrationCard)}
                            </div>
                          ) : (
                            <div className="p-6 text-center border-2 border-dashed border-border rounded-[24px]">
                                <p className="text-xs text-muted-foreground font-medium">Ingen eksterne tilmeldinger</p>
                            </div>
                          )}
                        </div>
                      )}
      ...
      {/* DETAIL SHEET FOR EVENT ANSWERS */}
      <FullscreenSheet
        open={!!selectedResultForDetail}
        onOpenChange={(o) => !o && setSelectedResultForAnswers(null)}
        title={selectedResultForDetail?.userName || 'Svar'}
        rightSlot={<button onClick={() => setSelectedResultForAnswers(null)} className="p-2 hover:bg-black/5 rounded-full transition-colors"><X className="h-6 w-6 opacity-40" /></button>}
      >
        <div className="p-6 space-y-6 pb-32 mx-auto max-w-2xl">
          <div className="flex flex-col items-center gap-4 text-center mb-4">
            <Avatar className="h-20 w-20 border-4 border-white shadow-md">
              <AvatarImage src={selectedResultForDetail?.photoURL || undefined} />
              <AvatarFallback className="text-xl">{getInitials(selectedResultForDetail?.userName)}</AvatarFallback>
            </Avatar>
            <div>
              <h3 className="text-xl font-bold">{selectedResultForDetail?.userName}</h3>
              <p className="text-sm text-muted-foreground">{selectedResultForDetail?.userEmail}</p>
            </div>
          </div>

          {(overviewItem?.formFields || []).map((field: any) => {
            const answer = selectedResultForDetail?.formData?.[field.id];
            return (
              <div key={field.id} className="space-y-2 p-5 rounded-3xl bg-card border border-border shadow-sm">
                <p className="text-xs font-bold uppercase tracking-widest text-muted-foreground">{field.label}</p>
                <p className="text-[17px] font-bold text-foreground leading-relaxed break-words">
                  {Array.isArray(answer) ? answer.join(', ') : (answer || 'Intet svar')}
                </p>
              </div>
            );
          })}
      ...
      <FullscreenSheet open={isAnnounceOpen} onOpenChange={setIsAnnounceOpen} title={editingItem ? t('editAnnouncement') : t('createAnnouncement')} rightSlot={<button onClick={() => setIsAnnounceOpen(false)} className="p-2 hover:bg-black/5 rounded-full transition-colors"><X className="h-6 w-6 opacity-40" /></button>}>
        <div className="p-6 space-y-8 pb-32 max-w-full">
              ...
              {bannerImagePreview ? (
                <div className="relative w-full aspect-video cursor-pointer" onClick={() => fileInputRef.current?.click()}>
                  ...
                  <div className="absolute inset-0 bg-black/40 flex flex-col items-center justify-center opacity-0 group-hover:opacity-100 transition-opacity">
                    ...
                    <span className="text-[10px] font-extrabold text-white uppercase tracking-widest">Klik for at skifte</span>
                  </div>
          ...
          <Button onClick={handleCreateOrUpdateAnnounce} disabled={isSubmitting} className="w-full h-16 rounded-2xl bg-[#2E9D63] text-white font-bold text-lg shadow-lg shadow-green-500/20">{isSubmitting ? <Loader2 className="mr-2 h-5 w-5 animate-spin" /> : <Megaphone className="mr-2 h-5 w-5" />} {editingItem ? t('saveAnnouncement') : t('sendAnnouncement')}</Button>
        </div>
      </FullscreenSheet>

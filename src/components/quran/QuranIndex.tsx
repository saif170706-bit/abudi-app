...
export default function QuranIndex({ onNavigate, BackButton }: QuranIndexProps) {
  ...
  if (isContextLoading) {
    return <div className="flex h-screen items-center justify-center bg-background"><Loader2 className="h-8 w-8 animate-spin text-[#004D40]" /></div>;
  }
  ...
  return (
    <div className="min-h-screen pt-12 pb-32 px-6 w-full max-w-lg mx-auto space-y-10">
      <motion.div initial={{ y: 20, opacity: 0 }} animate={{ y: 0, opacity: 1 }} className="flex items-center justify-between">
          <div>
              <h1 className="text-5xl font-display text-[#004D40] tracking-tight">{tGlobal('quran')}</h1>
              <p className="text-[10px] font-black uppercase tracking-[0.25em] text-[#DEA93E] mt-2">{tGlobal('Udforsk de hellige skrifter')}</p>
          </div>
          <div className="h-16 w-16 rounded-[24px] bg-[#004D40]/5 flex items-center justify-center border border-[#004D40]/10">
              <BookOpen className="h-8 w-8 text-[#004D40]" />
          </div>
      </motion.div>

      <Tabs defaultValue="surah" className="w-full">
        <TabsList className="grid w-full grid-cols-2 h-16 bg-[#004D40]/5 p-2 rounded-[28px] border border-[#004D40]/5">
          <TabsTrigger value="surah" className="rounded-2xl font-black uppercase text-[10px] tracking-widest data-[state=active]:bg-white data-[state=active]:text-[#004D40] data-[state=active]:shadow-lg active:scale-95 transition-all">
            {tGlobal('surah')}
          </TabsTrigger>
          <TabsTrigger value="page" className="rounded-2xl font-black uppercase text-[10px] tracking-widest data-[state=active]:bg-white data-[state=active]:text-[#004D40] data-[state=active]:shadow-lg active:scale-95 transition-all">
            {tGlobal('Side')}
          </TabsTrigger>
        </TabsList>
                ...
                {recentVisits.slice(0, 2).map(visit => (
                  ...
                  >
                    <div className="glass-card-inner !p-4 text-center">
                        <p className="font-bold text-[#004D40] truncate text-sm">{visit.surahName}</p>
                        <p className="text-[10px] text-[#DEA93E] font-black uppercase tracking-widest mt-1">{tGlobal('Side')} {visit.page}</p>
                    </div>
                  </motion.div>
                ))}
              </div>
          ...
          <div className="grid grid-cols-1 gap-4">
            {filteredSurahs.length > 0 ? (
              filteredSurahs.map((surah, idx) => (
                ...
                >
                  <div className="glass-card-inner !py-5 !px-6 flex items-center gap-5">
                    <div className="flex-shrink-0 w-12 h-12 rounded-2xl bg-[#004D40]/5 flex items-center justify-center font-display text-lg text-[#004D40] group-hover:bg-[#DEA93E] group-hover:text-white transition-all">
                      {surah.number}
                    </div>
                    <div className="flex-grow">
                      <h3 className="font-bold text-[17px] text-[#004D40]">{surah.englishName}</h3>
                      <p className="text-[10px] font-black uppercase tracking-widest text-[#004D40]/30 mt-1">
                        {surah.revelationPlace === 'Meccan' ? tGlobal('Makki') : tGlobal('Madani')} • {surah.versesCount} {tGlobal('vers')}
                      </p>
                    </div>
                    <div className="text-right">
                      <p className="font-quran text-3xl text-[#004D40] group-hover:text-[#DEA93E] transition-colors">{surah.arabicName}</p>
                    </div>
                  </div>
                </motion.div>
              ))
              ...
              <div className="py-20 text-center space-y-4">
                <div className="h-16 w-16 bg-[#004D40]/5 rounded-full flex items-center justify-center mx-auto">
                    <Search className="h-8 w-8 text-[#004D40]/20" />
                </div>
                <p className="text-[#004D40]/40 font-bold">{tGlobal('noResults').replace('{searchTerm}', searchTerm)}</p>
              </div>
            )}
          </div>
        </TabsContent>
        ...
        <TabsContent value="page" className="mt-12 outline-none">
          <motion.div initial={{ y: 20, opacity: 0 }} animate={{ y: 0, opacity: 1 }} className="max-w-xs mx-auto text-center space-y-10">
            <div className="space-y-4">
              <div className="h-24 w-24 bg-[#DEA93E]/10 rounded-[40px] flex items-center justify-center mx-auto shadow-inner">
                <BookOpen className="h-12 w-12 text-[#DEA93E]" />
              </div>
              <h3 className="text-3xl font-display text-[#004D40]">{tGlobal('goToPage')}</h3>
              <p className="text-[10px] font-black uppercase tracking-widest text-[#004D40]/30">{tGlobal('Vælg en side mellem 1 og 604')}</p>
            </div>
            ...
            <form onSubmit={handlePageSearch} className="space-y-6">
              ...
              />
              <Button type="submit" className="w-full h-20 rounded-[32px] text-lg font-black uppercase tracking-[0.2em] bg-[#004D40] text-white shadow-2xl active:scale-95 transition-all">
                {tGlobal('go')}
              </Button>
            </form>
          </motion.div>

...
export default function AvatarUploader() {
  ...
  const onFileChange = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (!file || !user || !firebaseApp || !firestore || !profile) return;

    // Validation
    if (!file.type.startsWith('image/')) {
      toast({ variant: 'destructive', title: 'Forkert filtype', description: 'Vælg venligst et billede (png, jpg, etc.).' });
      e.target.value = '';
      return;
    }
    if (file.size > 5 * 1024 * 1024) {
      toast({ variant: 'destructive', title: 'For stor fil', description: 'Billedet må højst fylde 5MB.' });
      e.target.value = '';
      return;
    }
    ...
    try {
      ...
      if (profile.photoURL && profile.photoURL.includes('firebasestorage.googleapis.com')) {
        try {
          // Extract path from URL or use a simple ref if possible
          // But a safer way is to just use ref(storage, currentPhotoURL)
      ...
      // Success toast removed as requested by user for a cleaner UI experience
    } catch (err: any) {
      console.error('Avatar upload failed:', err);
      toast({
        variant: 'destructive',
        title: 'Upload fejlede',
        description: 'Der opstod en fejl under upload af billedet.',
      });
    } finally {
      setUploading(false);

    ...
    <View className="gap-4">
      {questions.map((question, index) => (
        <View key={question.id} className="gap-3 rounded-xl border border-border bg-background p-3">
          ...
          <Input
            placeholder={tGlobal('Spørgsmålstekst')}
            value={question.text}
            onChangeText={(v) => updateQuestion(question.id, { text: v })}
          />
          <ChipPicker
            options={QUESTION_TYPE_OPTIONS.map((o) => ({ ...o, label: tGlobal(o.label) })) as any}
            value={question.type}
      ...
      <Pressable onPress={() => onChange([...questions, createEmptyQuestion()])}>
        <Text className="text-sm font-medium text-primary">{tGlobal('+ Tilføj spørgsmål')}</Text>
      </Pressable>
    </View>
  );
}
